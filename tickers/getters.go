...
func FTXGetter(ticker string) (float64, error) {
	url := "https://ftx.com/api/markets/" + ticker
	body, err := getHTTPResponseBodyFromUrl(url)
	if err != nil {
		return 0, err
	}
	var tickerData FTXTicker
	err = json.Unmarshal(body, &tickerData)
	if err != nil {
		return 0, err
	}
	return (tickerData.Result.Bid + tickerData.Result.Ask) / 2, nil
}
